...
export interface TargetPowerFieldProps {
  value: number;
...
export default function TargetPowerField({
  value,
  onChange,
  onCalculate,
  onRandom,
}: TargetPowerFieldProps) {
  const { t } = useI18n();
    ...
    <SidebarSection icon="target" title={t('targetPower')} className="space-y-4">
      <div className="space-y-2">
        <div className="flex justify-between">
          <label htmlFor="target-power-input" className="text-sm text-endfield-text">
            {t('power')} (w)
          </label>
          <span className="text-sm text-endfield-yellow" aria-live="polite">
            {value}
          </span>
        </div>
